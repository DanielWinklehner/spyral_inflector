...
from spyral_inflector.spyral_inflector import SpiralInflector
...
t_inflector_defined = time.time()

# si.load_bfield()
si.load_bfield(bfield=Field(dim=0, field={"x": 0.0, "y": 0.0, "z": -1.04}))

t_load_field = time.time()

si.initialize()

t_initialized = time.time()
...
si.solve(use_gpu=False)

t_geometry_solved = time.time()
...
t_field_solved = time.time()

print("Calculating field took {:.4f} s".format(time.time() - t_code_start))
...
t_bunch_defined = time.time()

print("LOAD LIBRARIES ---------- {:.4f} s".format(t_inflector_defined - t_code_start))
print("LOAD FIELDS ------------- {:.4f} s".format(t_load_field - t_inflector_defined))
print("INFLECTOR INITIALIZED --- {:.4f} s".format(t_initialized - t_load_field))
print("INFLECTOR GEO. SOLVED --- {:.4f} s".format(t_geometry_solved - t_initialized))
print("INFLECTOR FIELD SOLVED -- {:.4f} s".format(t_field_solved - t_geometry_solved))
print("DEFINE PARTICLE BUNCH --- {:.4f} s".format(t_bunch_defined - t_field_solved))
...
print("PARTICLE TRACKING ------- {:.4f} s".format(t_tracked - t_bunch_defined))
