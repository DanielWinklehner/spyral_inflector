...
$cand   = "__CAND__"                      # geometry name (for the report)
$knobs  = @(__KNOBS__)                    # e.g. "--quad-bore","0.016","--aper-hole","0.0155","--slot-width","0.019"
$orient = @(__ORIENT__)                   # @("--swap-xy") or @()
$phiC   = __PHIC__                        # centre beam angle [deg] on top of the orientation
$qc1    = __QC1__                         # quad-1 voltage centre [V]
$qc2    = __QC2__                         # quad-2 voltage centre [V]
$qw     = __QW__                          # Phase B inner grid half-width [V]
$qn     = __QN__                          # Phase B inner grid points per axis
...
$g1 = "$deck\Results\wiggle\__RUN1__"; $s1 = "$deck\Geometry\wiggle\__RUN1___steps"
if (Test-Path "$g1\bunch_bunch.npz") {
    Stamp "Phase B: __RUN1__ already complete, skipping the geometry run"
} else {
    python -u "$scripts\GeometryPoint.py" --name __RUN1__ @knobs @orient --phi $phiC --rotate-quads 0 0 --res 0.0025 `
        --q1 ($qc1 - $qw) ($qc1 + $qw) $qn --q2 ($qc2 - $qw) ($qc2 + $qw) $qn --n-scan 1500 --n-bunch 5000 2>&1 |
        Out-File -Encoding utf8 "$fin\log___RUN1__.txt"
    Get-Content "$fin\log___RUN1__.txt" | Select-String -Pattern "design energy|optimizer:|\] done:|failed" | ForEach-Object { Stamp $_.Line }
}
...
Stamp "Phase C2: quad rotations __A1__ x __A2__ deg, 3x3 at 750 V"
python "$scripts\scan_complete.py" "$g1\scan2_c2_alpha.json" __C2N__ | Out-Null
if ($LASTEXITCODE -eq 0) {
    Stamp "C2: finished scan reused"
} else {
    python -u "$scripts\BunchScan2.py" --tag c2_alpha @scan --phi $phi --alpha1 __A1__ --alpha2 __A2__ `
...
$g2 = "$deck\Results\wiggle\__RUN2__"; $s2 = "$deck\Geometry\wiggle\__RUN2___steps"
if (Test-Path "$g2\bunch_bunch.npz") { Stamp "Phase D: __RUN2__ geometry already complete, reused" } else {
python -u "$scripts\GeometryPoint.py" --name __RUN2__ @knobs @orient --phi $phi --rotate-quads $a1 $a2 --fix-dz $dz2 --res 0.0025 `
    --q1 ($q1 - 375) ($q1 + 375) 3 --q2 ($q2 - 375) ($q2 + 375) 3 --n-scan 1500 --n-bunch 5000 2>&1 | Out-File -Encoding utf8 "$fin\log___RUN2__.txt"
Get-Content "$fin\log___RUN2__.txt" | Select-String -Pattern "optimizer:|\] done:|failed" | ForEach-Object { Stamp $_.Line }
}
$r = Best "$g2\scan2_inner.json"; $q1 = [double]$r[3]; $q2 = [double]$r[4]
$V2 = [double](JsonValue "$g2\summary.json" "d['optimizer']['voltage_V']")
$spiralV2 = $V2 * $vs
Stamp "__RUN2__: quads $q1/$q2 V, spiral $V2 V x $vs = $spiralV2 V; direct solve + 10k bunch"
...
Stamp "report"
python "$scripts\make_final_report.py" --fin "__FIN__" --run1 "__RUN1__" --run2 "__RUN2__" --title "__TITLE__" 2>&1 | ForEach-Object { Stamp $_ }
Stamp "done"
