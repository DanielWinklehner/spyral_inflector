...
#   powershell -File RunSCLevel.ps1 [-Run vfocus1_final] [-Package "<repo path>"] [-BestJson sc_retune\sc_grid.json] [-WaitFor sc_retune.md]
...
while (-not (Test-Path (Join-Path $rundir $WaitFor))) { Start-Sleep -Seconds 60 }
"[$(Get-Date -Format HH:mm:ss)] $WaitFor present; levelling $Run at the quad setting of $BestJson" | Out-File -Append -Encoding ascii $log
