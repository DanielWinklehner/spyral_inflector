...
MACHINE_FRAME = ("machine frame: origin at the machine centre in the median plane, +z up (towards the RFQ), +x = azimuth 0 "
                 "on a magnet hill, azimuth counter-clockwise from above, beam coming down the axis and circulating "
                 "counter-clockwise, B_z < 0; = the deck frame mirrored through the median plane (z -> -z, pz -> -pz); "
                 "SI units, momenta in eV/c")


def save_handoff_openpmd(path, plane, sel, ion, rf_hz, current_ma, trj, vdes, handoff_distance_m,
                         phase_reference="mean", extra_meta=None, mode="plane_crossing", frame="deck"):
    ...
    and the plane definition as attributes. frame="machine" mirrors everything through the
    median plane (z -> -z) before writing. Returns a dict with the numbers written."""
    ...
    r, v, t = plane.state[sel, :3].copy(), plane.state[sel, 3:].copy(), plane.time[sel]
    point, normal = np.array(plane.point, dtype=float), np.array(plane.normal, dtype=float)
    exit_point, exit_vel = np.asarray(trj[-1], dtype=float).copy(), np.asarray(vdes[-1], dtype=float).copy()
    if frame == "machine":
        for arr in (r, v):
            arr[:, 2] *= -1.0
        point[2] *= -1.0
    ...
    u_ax, v_ax, w_ax = plane_axes(normal)
    d = r - point
    n = int(sel.sum())
    ...
    meta = dict(bunch_charge=q_bunch, bunch_freq=rf_hz, time=t, status=np.ones(n, dtype=int),
                ...
                mode=mode, plane_origin_m=point, plane_normal=w_ax, plane_axis_u=u_ax, plane_axis_v=v_ax,
                ...
                handoff_definition=("plane through the design particle's continued orbit at handoff_distance_m of path "
                                    ...
                                    "handoff_distance_m = 0 is the electrode exit plane itself, inside the electric fringe"),
                phase_reference_time_s=t_ref, rf_frequency_hz=float(rf_hz), beam_current_ma=float(current_ma),
                design_exit_point_m=exit_point, design_exit_velocity_mps=exit_vel,
                frame=(MACHINE_FRAME if frame == "machine" else FRAME), frame_name=frame,
...
def save_snapshot_openpmd(path, snap, sel, ion, rf_hz, current_ma, extra_meta=None, frame="deck"):
    """Write a 6-D lab-frame snapshot (r, v, active, t) of the selected particles to openPMD
    (frame="machine": z and v_z mirrored)."""
    from PyPATools.particles_src.particle_io import save_openpmd
    r, v, active, t = snap
    ...
    meta = dict(bunch_charge=q_bunch, bunch_freq=rf_hz, time=np.full(n, t), status=np.ones(n, dtype=int),
                ...
                frame=(MACHINE_FRAME if frame == "machine" else FRAME), frame_name=frame, n_particles=n)
    if extra_meta:
        meta.update(extra_meta)
