...
class SettingsHandler(object):
    ...
    Preferred: $HOME/.local/lib/python<version>/site-packages/spyral_inflector (Linux)
               $APPDATA/spyral_inflector (Windows)  (usually $APPDATA=C:\Users\<username>\AppData\Roaming)
    Fallback: Wherever the module is installed
              e.g. ...Anaconda3\envs\<env name>\Lib\site-packages\spyral_inflector
    The handler looks in the preferred directory first, so the user can override the default settings
    by creating the spyral_inflector directory and putting a copy of Settings.txt in there.
    """
    ...
    def __init__(self):
        ...
        if self._unix:
            self._path1 = "{}/.local/spyral_inflector".format(os.environ["HOME"])
        else:
            self._path1 = os.path.join("{}".format(os.environ['APPDATA']), "spyral_inflector")
    ...
    def load_settings(self):
        ...
        Preferred: $HOME/.local/lib/python<version>/site-packages/spyral_inflector
